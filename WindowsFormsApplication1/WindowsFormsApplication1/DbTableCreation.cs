...
namespace WindowsFormsApplication1
{
    class DbTableCreation
    {
        ...
        private static string connectionStr2 = String.Format("Server=127.0.0.1;Port=5432;" +
        ...
        static public void createLeagueTable(string tableName, string[] playerslist){
            string createTable;

            NpgsqlConnection conn = new NpgsqlConnection(connectionStr2);
            conn.Open();
            NpgsqlCommand command = new NpgsqlCommand(createTable, conn);
            command.ExecuteNonQuery();
            conn.Close();
